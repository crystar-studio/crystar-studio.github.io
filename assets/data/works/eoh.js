/**
 * END OF HOPE:21xx - Last Call　作品データ
 *
 * 画像は名前だけ書けばよく、Resources/eoh/eoh_(名前).png に解決される。
 * 解決の規約は assets/data/works.config.js の resourceOf() にある。
 */
export default {
  /* ============ STORYブロック ============ */
  story: {
    visuals: ['story_01', 'story_02', 'story_03'],

    text: `『滅びたこの星に、救いはあるのか――』

かつて数々の厄災が地球を襲い、灰色の死の星と化した。大半の人類はスペースコロニーへと逃れたが、すべての人間がそこへ行けたわけではなかった。

地上に取り残された者たちは、生存のために"変異体"と呼ばれる存在のもとに集まり、コロニーを形成していた。異能、魔法、科学、呪術——それぞれの力を駆使しながら、独自の目的を掲げ、終末世界でもがいている。

集団の一つ、あぶれ者たちが集まった《淵》に主人公・ルイは所属し、"希望"を求めて終末世界を彷徨っていた。

――だが、彼らの"希望"を探す旅は、ここで、終わりを迎えようとしている。`,

    link: { label: 'GO', href: 'https://ncode.syosetu.com/n2974kb/' },
  },

  /* ============ SERIESブロック ============ */
  /* 並べるのは作品コードだけ。カードの中身は works.config.js の作品定義から取る */
  series: ['eoh_lost_star', 'eoh_silent_ember'],

  /* ============ WORLDブロック ============ */
  world: {
    visuals: ['world_01', 'world_02'],

    text: `二十一世紀末。度重なる天災と、二度にわたる地球外生命体の襲来。地球は灰の雲に覆われ、極寒の死の星と化した。

人類は《方舟計画》によってスペースコロニーへの脱出を図ったが、乗ることができたのはごく一部だった。計画は終了し、地上には行き場を失った者たちだけが残されている。

残された者たちは、それぞれの力を頼りに生き延びる術を探した。世界の理を書き換える《異能》。思い描いた事象を現実に上書きする《魔法》。そして、人を超えた存在――《変異体》。

地上には、いくつかの生存圏が点在する。

・京都の山岳地帯、霧に閉ざされた渓谷の奥に隠された秘境《澄幽》。変異体ミコの加護に守られた、この世界で最後の安息の地。戦闘組織《淵》がこれを守る。
・東ヨーロッパの廃墟に生まれた、異能者たちの無法地帯《ヴァルケイア》。法はなく、唯一の秩序は力である。
・中国沿岸に興った魔法秩序都市国家《皓絳》と、それを統べる機関《天衡院》。閉ざされ、内情を知る者はほとんどいない。

荒野には星骸と呼ばれる怪物が徘徊し、異能者と魔法士は互いを狩り続けている。この星に、安全な場所はもう残されていない。`,

    characters: ['louis', 'xiya', 'miou', 'falon', 'maqi', 'belyisvet', 'obsidia'],
  },

  /* ============ 用語集（terms.html?code=eoh） ============ */
  /* WORLDブロックの下のGLOSSARYボタンから開く。シリーズ作品もこの一覧を共有する。
     term  … 見出しになる語
     reading / aliases … 検索でひっかけたい読みや別名（表示するのは reading だけ）
     text  … 最初から見えている説明
     reveals … 物語の進行で変わる情報。書いた順に入れ子で開くので、
               1段目を開くまで2段目は見えない。段数は何段でもよい（5段でも可）。 */
  terms: [
    {
      term: '世界',
      text: `地球が滅びへ向かい、人類の大半が切り捨てられた終末世界。異能者の存在によって環境や秩序が容易に崩壊する。`,
    },
    {
      term: '救い',
      text: `ない。「幸福な死」さえ、残される者の痛みを考えれば救いにはならない。`,
    },
    {
      term: '安息',
      text: `完全な救済ではなく、誰かと生きている瞬間にだけ手の中に残る、わずかな穏やかさ。`,
    },
    {
      term: '地球',
      text: `物語の主な舞台となる荒廃した地表。
すでに「緑の惑星」としての姿を失いつつある、人類の旧来の故郷。地上には取り残された人々が暮らす。猛吹雪、異能災害、資源不足などに脅かされる。`,
    },
    {
      term: '異能者',
      text: `異能を持つ人間。魔法は使えず、その強大な力への恐怖から、方舟への乗船も認められなかった。ルイも異能者の一人である。`,
    },
    {
      term: '異能',
      text: `世界の理そのものを捻じ曲げ、書き換える力。時を巻き戻す、この世にない空間を生み出すなど、神の領域に届くほど強大であり、物に向ければ文明を支える力にも、人に向ければ容易に命を奪う力にもなる。`,
    },
    {
      term: '魔法',
      reading: 'まほう',
      text: `異能とは異なり、世界の理の上から現実を塗り替える力。異能者は魔法を使えず、魔法士は異能を持たないという、覆らない理がある。`,
    },
    {
      term: 'ルイの異能',
      aliases: ['支配', 'ドミネート'],
      text: `《支配／ドミネート》
ルイが発動した異能。死んだ異能者の力を利用し、吹雪を消し去った。対象の異能を掌握・行使する力と考えられる。`,
    },
    {
      term: '星骸',
      text: `外界を徘徊する脅威のひとつ。巨大な身体を持つ個体もおり、人間を容易に殺傷する。異能者・魔法士と並び、荒野で出会えば死に直結する存在。構成物は、地球外生命体の死骸と岩石など。`,
    },
    {
      term: '隕石',
      text: `昨晩、空から落ちてきたと見られていたもの。現場には欠片すら残っていなかった。`,
    },
    {
      term: '澄幽',
      reading: 'ちょうゆう',
      text: `ミコのエネルギーによって維持されている、外界の災厄から隔てられた集落・拠点。春のような気温、霧、偽りの空を保つが、ミコの衰弱に伴い霜や寒気といった変調が現れ始めている。`,
    },
    {
      term: '澄幽イケメンランキング',
      text: `澄幽の住民の間で行われた、男性の容姿を順位づけする催し。珠桜が全票を得て優勝したことがある、らしい。`,
    },
    {
      term: 'スペースコロニー',
      text: `地球を捨てた人類が宇宙に建設した新たな居住地。人工の自然環境と生活基盤を備え、数千万人規模の人々を収容する。`,
    },
    {
      term: '方舟',
      text: `地球とスペースコロニーを行き来し、人々を運んでいた輸送手段。数年前に運行を停止し、地上に残る者の脱出路を断った。異能者は、その危険性から乗船を拒まれていた。`,
    },
    {
      term: '楽園',
      text: `スペースコロニーを指す比喩。表面上は安全で豊かな生活圏だが、基準に満たない者を容赦なく排除する。`,
    },
    {
      term: '船外放逐',
      text: `コロニーから排除される者への処分の一つ。文字通り宇宙空間へ放り出される、極めて苛烈な手段。犯罪者、病弱者、繁栄の妨げと判断された者、異能者などが対象となる。`,
    },
    {
      term: '寧静亭',
      reading: 'ねいせいてい',
      text: `珠桜の執務室が置かれている建物。外見以上に複雑で、執務室の奥には一般住民に秘匿されてきた区画がある。`,
    },
    {
      term: 'ミコ',
      text: `澄幽を維持する力の源である「変異体」の少年。絶えずエネルギーを放出しており、澄幽の春、谷の霧、偽りの陽・月はすべて彼女の身体から零れた力によるもの。未来視の異能を持つ。`,
    },
    {
      term: '変異体',
      text: `ある日を境に、絶えずエネルギーを放ち続ける身体になった人間の呼称。滅びかけた地球で人々の生活を支える、希少かつ不可欠な存在。`,
    },
    {
      term: 'ミコの異能',
      text: `ミコに宿る、未来の破滅を先取りして受ける代償として、その瞬間の痛みを受ける力。視た未来は変えられず、変えようとした行動すら未来を成立させる方向へ収束する。`,
    },
    {
      term: '澄幽の偽りの陽／月',
      text: `ミコの力が生み出す、澄幽の昼夜を形作る人工の天体。空の青も含め、外界の灰色の雲から住民を守る仮初の景観。`,
    },
    {
      term: '澄幽の霧',
      text: `ミコから零れたエネルギーが、谷や森を覆う形で現れたもの。寝台の周囲に漂う薄桃色の光の粒と同質。疲労回復効果がある。`,
    },
    {
      term: '白い光',
      text: `ミコが視た滅びの象徴。空の船々を一隻残らず焼き、その後は地上の澄幽にも到達するとのこと。具体的な日時やその正体は明かされていない。`,
    },
    {
      term: '東の門の夜',
      text: `過去にミコが予知した災厄。珠桜が避難・防衛を徹底しても偶然が重なり、視たとおりに門が炎に呑まれ、多くの人が死亡した。未来が変わらないことを決定づけた事件。`,
    },
    {
      term: '黒華家',
      reading: 'くろはなけ',
      text: `珠桜が継ぐ家名・系譜。`,
    },
    {
      term: 'ルイの試練',
      text: `ルイに訪れるとミコが警告した出来事。『それは、あなたが忘れてきたもの。あなたの後ろに、ずっと、ついてきたもの。逃げても、それは追ってくるでしょう。あなたの、大切なものごと』`,
    },
    {
      term: 'ルイの空白の過去',
      text: `ルイが失っている十一歳以前、特に五歳から十一歳までの記憶。鎖に繋がれた断片の記憶の次は、珠桜に手を引かれ、澄幽の土を初めて踏んだ時の記憶。`,
    },
    {
      term: '「過去に立ち向かって」',
      text: `ミコがルイへ残した最後の警告。逃げられない過去と向き合うことが、絶望を越えるための条件として示される。`,
    },
    {
      term: '翠霞庵',
      reading: 'すいかあん',
      text: `澄幽にある施設。温泉や食堂などが入っており、まとめて女将の藤崎が営んでいる。住民が身体を休め、交流する生活の場でもある。`,
    },
    {
      term: '月風門',
      reading: 'げっぷうもん',
      text: `澄幽と外界を隔てる門。ルイが十五歳の冬に初めて外へ出た場所であり、門の内外で春のような澄幽と凍てつく荒野の落差が明確に現れる。`,
    },
    {
      term: '鎮魂の杜',
      reading: 'ちんこんのもり',
      text: `澄幽で亡くなった者たちを弔う墓地。千を超える墓石が並び、区画ごとに花が植えられている。死者の名と、この地で生きた証を残す場所。`,
    },
    {
      term: '準氷河期',
      reading: 'じゅんひょうがき',
      text: `外界を長年覆っている極寒の気候状態。一年のほとんどが氷点下で、地上には厚い雲が広がり、陽光はほとんど届かない。`,
    },
    {
      term: '外界',
      reading: 'がいかい',
      text: `澄幽の境界の外に広がる、灰と氷に覆われた荒野。放射線、酸性雨、有毒霧、吹雪、星骸・異能者・魔法士などの脅威がある。`,
    },
    {
      term: '魔法士',
      reading: 'まほうし',
      text: `魔法師のうち、天衡院に属する者を指す言葉。「魔法を扱う兵士」という意味。`,
    },
    {
      term: '前哨基地',
      reading: 'ぜんしょうきち',
      text: `魔法士が遠方の探索に出る際に築く拠点。二年前には列島の西にも建てられた。澄幽を探し出して消し去るための足がかりであり、シアはその内部で被検体として扱われていた。`,
    },
    {
      term: '二人の主力',
      text: `魔法士の前哨基地攻略で正面戦闘を担った、澄幽の戦闘要員二名。作戦後は再び外へ出ており、二年間帰還していない。`,
    },
    {
      term: '試刃院',
      reading: 'しじんいん',
      text: `鍛治士シンが籠もっている工房を併設した訓練場。`,
    },
    {
      term: '調査部隊',
      reading: 'ちょうさぶたい',
      text: `澄幽の外へ出て、調査・戦闘・救助などを行う部隊。現在の所属者はルイ、シアを含む四人と、二年間帰還していない主力二人のみである。`,
    },
    {
      term: '異能殺人犯',
      reading: 'いのうさつじんはん',
      text: `十四年前、世界で初めて異能を人へ向けて殺人を行った男。彼の存在を契機に、異能者全体が恐怖と警戒の対象になった。`,
    },
    {
      term: 'ルイのエネルギー弾',
      text: `通常の弾丸では傷つけられない星骸の外殻を貫くために造られた特殊な弾。ルイの双銃に装填され、星骸の撃破に用いられる。`,
    },
    {
      term: 'ルイの双銃',
      text: `ルイが外で使用する二丁の銃。星骸へ異能のエネルギー弾を放つほか、銃身下部の刃を展開してダガーとしても扱える。`,
    },
    {
      term: '軽装派',
      reading: 'けいそうは',
      text: `調査部隊内にある装備方針の一つ。攻撃を受ければ防具の有無にかかわらず致命的だとして、防具を最小限に抑える考え方。ルイはこちらに属する。`,
    },
    {
      term: '重装派',
      reading: 'じゅうそうは',
      text: `調査部隊内にある装備方針の一つ。生存率を少しでも上げるため、一枚でも多く防具を着込むべきだとする考え方。`,
    },
    {
      term: 'ルイのゴーグル',
      text: `珠桜がルイへ、調査部隊加入時に渡した防寒具。無数の傷がついても使い続けており、ルイにとって手放せない最初の装備。`,
    },
    {
      term: '狭間',
      reading: 'はざま',
      text: `何者かが「友人」に用意させた、現実から切り離された異空間。内部では距離・時間・知覚・身体の自由といった概念が崩れ、外へ出れば、その中で起きた事象はなかったことになる。`,
    },
    {
      term: '女神',
      reading: 'めがみ',
      text: `金の糸で織り上げられた女性的な貌。絶えず編み直されて顔立ちが変化するが、その背後にはルイを「親友の子供」と呼ぶ男の意志がある。`,
    },
    {
      term: 'ナティ',
      text: `ルイの父の愛称？　狭間でルイが遭遇した男が「死んだナティ」「大切な親友」と呼んだ。`,
    },
    {
      term: '狭間の男',
      text: `ルイを狭間へ引き込み、金の糸を通じて肉体と知覚を支配した正体不明の人物。ナティを親友と呼び、ルイを父の代わりに庇護・所有しようとする。`,
    },
    {
      term: '「必ず迎えに行く」',
      text: `狭間の男がルイへ告げた言葉。ルイの意思にかかわらず、自分のもとへ連れ戻すという男の確信と執着を示す。`,
    },
    {
      term: 'シアの魔法',
      text: `シアが思い描き、願った事象を現実へ上乗せする力。発動の前触れがほとんどなく、並外れた速度で構築・発動できる一方、彼女が明確に願えない「殺す」という結果には届かない。`,
    },
    {
      term: '魔法師',
      reading: 'まほうし',
      text: `魔法を使う者を指す言葉。かつて澄幽や調査部隊にも所属者がいた。魔法"士"とは意味が異なる。`,
    },
    {
      term: '蜘蛛型星骸',
      reading: 'くもがたせいがい',
      text: `複数の長い脚と無数の複眼を持つ星骸。洞窟のような狭所では巨体が動きを制限される一方、腐食性の糸を放つ。瓦礫の隙間から小型個体が多数現れる。`,
    },
    {
      term: '巨人型星骸',
      reading: 'きょじんがたせいがい',
      text: `十メートルを超える体躯を持つ、人型に近い星骸。黒曜石のような外殻と発光する関節を備え、圧倒的な質量と速度で対象を圧殺する。外殻の継ぎ目や、内部にある光る芯が攻撃の要所となる。`,
    },
  ],

  /* ============ 陣営（CHARACTERブロックのタブ） ============ */
  characterGroups: [
    { id: 'choyu', label: '澄幽' },
    { id: 'kokou', label: '皓絳' },
    { id: 'valkeia', label: 'ヴァルケイア' },
    { id: 'henitai', label: '変異体' },
  ],

  /* ============ キャラクター ============ */
  characters: [
    /* ---------- 澄幽・メイン ---------- */
    {
      id: 'louis',
      group: 'choyu',
      color: '#32CD32',

      name: 'ルイ',
      realName: 'ルイ・クレルヴォー',
      alphabet: 'LOUIS CLAIRVAUX',
      quote: 'シアの魔法を、"偽物"呼ばわりするな！',

      affiliation: '《淵》調査部隊',
      gender: '男性',
      age: '19',
      birthday: '2088年6月24日',
      bloodType: 'O',
      height: '170cm',
      weight: '58kg',
      firstPerson: '俺',
      residence: '澄幽・静月舎',

      intro: 'フランス出身。ライムグリーンの輝くような瞳を持つ、《淵》調査部隊の青年。四年目。星骸相手なら余裕を持って立ち回れるが、世界を変えるほどの相手に一人で挑む力はまだない。穏やかで冷静、決して他者を軽んじず、相手を「お前」ではなく名前で呼ぶ。ダガーと小型銃に変形するツインブレイザーの二丁使いで、射撃の腕は澄幽随一。「自分は弱い」と自覚しているからこそ、ひたむきに努力を重ねる。一人で抱え込む癖と、隠し事をするとき目を逸らす癖がある。',
    },
    {
      id: 'xiya',
      group: 'choyu',
      color: '#ff00ff',

      name: 'シア',
      realName: '柳曦芽',
      alphabet: 'LIU XIYA',
      quote: '絶対、一人になんてさせないから！',

      affiliation: '《淵》調査部隊',
      gender: '女性',
      age: '19',
      birthday: '2088年7月7日',
      bloodType: 'B',
      height: '156cm',
      weight: '45kg',
      firstPerson: '私',
      residence: '澄幽・翠霞庵',

      intro: '白銀の髪とアメジストの瞳を持つ少女。天衡院の前哨基地で実験体として扱われていたところを、二年前ルイに救出された。窓の外を眺めるだけだった日々の反動か、重力すら縛らない果てのない空に強く憧れている。魔法の構築速度は人並み外れて速く、防御と支援において誰よりも頼りになる。ただし優しすぎるがゆえに敵を害する意志を持てず、その迷いがそのまま威力の甘さとなって表れてしまう。守られるだけではなく、誰かを守れるようになりたいと願っている。',
    },
    {
      id: 'miou',
      group: 'choyu',
      color: '#262325',

      name: '黒華珠桜',
      realName: '',
      alphabet: 'MIOU KUROHANA',
      quote: '無理はしないで、ちゃんと体を休めるんだよ。これは命令',

      affiliation: '澄幽／《淵》統率者',
      gender: '男性',
      age: '31',
      birthday: '2076年3月26日',
      bloodType: 'O',
      height: '174cm',
      weight: '66kg',
      firstPerson: '私',
      residence: '澄幽・寧静亭',

      intro: '澄幽の実質的な指導者。漆黒の髪と瞳を持つ、静かで美しい人物。この地に集った人々を守り続けている。武術の達人であり、ルイの体術はすべて彼から教わったもの。穏やかで、誰に対しても分け隔てなく優しい。ただし鍛錬の場では容赦がなく、春の陽光のように暖かく、深淵のように情け容赦のない笑みを浮かべる――「慈悲深き悪魔」と呼ぶほかない一面を覗かせる。',
    },
    {
      id: 'ritto',
      group: 'choyu',
      color: '#6A1435',

      name: '夜霧律灯',
      realName: '',
      alphabet: 'RITTO YAGIRI',
      quote: '僕も戦えますから',

      affiliation: '澄幽／珠桜の補佐',
      gender: '男性',
      age: '25',
      birthday: '2082年5月14日',
      bloodType: 'A',
      height: '175cm',
      weight: '61kg',
      firstPerson: '僕',
      residence: '澄幽・寧静亭',

      intro: '珠桜の補佐。冷静沈着で、常に礼を失わない物腰。澄幽の中枢を実務面から支えている。目立たぬ存在であることを選んできたが、決して無力ではなく、いざとなれば自ら前に出る。主人の命令は絶対だが、それ以上に、主人を危険に晒すことを何より嫌う。',
    },
    {
      id: 'hibiya',
      group: 'choyu',
      color: '#664d56',

      name: '夜霧響哉',
      realName: '',
      alphabet: 'HIBIYA YAGIRI',
      quote: 'ま、強けりゃなんでもいいんだよこの世界',

      affiliation: '《淵》調査部隊／"黎冥"',
      gender: '男性',
      age: '27',
      birthday: '2080年8月14日',
      bloodType: 'AB',
      height: '183cm',
      weight: '90kg',
      firstPerson: '俺',
      residence: '澄幽・黎冥の居',

      intro: '《淵》調査部隊、"黎冥"の片割れ。紫檀色の髪と銀灰色の瞳を持ち、鍛え上げられた肉体には無数の戦傷が刻まれている。珠桜に仕え、琴葉と組んで荒野を駆ける。どんな武器でも扱える適応力と、恐れずに踏み込んで勝ちをもぎ取る実戦力を持つ、澄幽最強格の一人。ルイとシアには軽口を叩く頼れる兄貴分として接し、よく笑う。ただしその笑い方には、どこか作り物めいたところがある。',
    },
    {
      id: 'kotoha',
      group: 'choyu',
      color: '#8B0000',

      name: '黒華琴葉',
      realName: '？？？',
      alphabet: 'KOTOHA KUROHANA',
      quote: '自衛もできないなら外に出ないで',

      affiliation: '《淵》調査部隊／"黎冥"',
      gender: '女性',
      age: '？',
      birthday: '2075年12月5日',
      bloodType: '？',
      height: '165cm',
      weight: '54kg',
      firstPerson: '私',
      residence: '澄幽・黎冥の居',

      intro: '《淵》調査部隊、"黎冥"の片割れ。夜空を切り取ったような漆黒の髪と深紅の瞳、傷ひとつない陶器のような白い肌を持ち、その所作は精密な時計のように無駄がない。魔法の構築力は常軌を逸しており、戦闘の最中に新たな魔法を組み上げ、「二度同じ魔法は使わない」と言われるほど。冷徹、簡潔、的確。ここ二年ほど、澄幽に帰っていない。',
    },

    /* ---------- ヴァルケイア ---------- */
    {
      id: 'falon',
      group: 'valkeia',
      color: '#C0A060',

      name: 'ファロン',
      realName: 'ファロン・ヴォーベール',
      alphabet: 'FALON VAUBÈRE',
      quote: '必ず迎えに行く',

      affiliation: 'ヴァルケイア／無窮の咎',
      gender: '男性',
      age: '36',
      birthday: '2071年4月28日',
      bloodType: 'AB',
      height: '197cm',
      weight: '79kg',
      firstPerson: '私',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: 'かつて《イージス・コンコード》の高官を務めた男。異能《Altération》を操り、あらゆる物質を望む形へと変質させる。ただしその力は「無」からは何も生み出せず、常に素材を必要とする――人間は、恰好の素材だ。',
    },
    {
      id: 'miredina',
      group: 'valkeia',
      color: '#12a3e6',

      name: 'ミレディーナ',
      realName: 'ミレディーナ・ティフォン',
      alphabet: 'MIREDINA TYPHON',
      quote: 'この物語は、私が幕を下ろすまで',

      affiliation: 'ヴァルケイア／無窮の咎',
      gender: '女性',
      age: '29',
      birthday: '2078年8月24日',
      bloodType: 'B',
      height: '173cm',
      weight: '53kg',
      firstPerson: 'ワタクシ',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: '異能《Cataclysm》によって天災そのものを操る異能者。地震、津波、火山噴火、雷撃――理論上起こりえない規模の災厄を引き起こす。青髪と赤い瞳を持ち、女王のような威厳をまとう。彼女の領域「サンクチュアリ・オブ・リバース」に広がる穏やかな雪も、そよ風も、オーロラも、すべて彼女の気分のままに姿を変える。無窮の咎の中では比較的穏やかとされるが、その機嫌ひとつが数億の命を左右する。',
    },
    {
      id: 'rita',
      group: 'valkeia',
      color: '#ff1500',

      name: 'リタ',
      realName: 'リタ・クレスト',
      alphabet: 'RITA KREST',
      quote: '新しいお友達、つくろーよー！',

      affiliation: 'ヴァルケイア／無窮の咎',
      gender: '女性',
      age: '17？',
      birthday: '2090年10月13日',
      bloodType: 'B',
      height: '150cm',
      weight: '40kg',
      firstPerson: 'ボク',
      residence: 'オルター・オブ・ザ・ディパーティッド',

      intro: 'プラチナブロンドの髪に黒い眼帯、病的なほど白い肌をした少女。鈴を転がすような声で、夕食の献立を話すような軽さで殺戮を語る。血を媒介とした異能《Subjugate》によって他者の精神を支配し、意のままに操る。彼女にとって人を壊すことも殺すことも「遊び」であり、退屈こそが最大の敵。エゼキエル以外に、彼女の相手を辛抱強く務められる者はこの世界にいない。',
    },
    {
      id: 'ezechiel',
      group: 'valkeia',
      color: '#1a2a6c',

      name: 'エゼキエル',
      realName: 'エゼキエル・ファルナスティア',
      alphabet: 'EZECHIEL FARNASTIER',
      quote: 'リタが怪我するの……やだ',

      affiliation: 'ヴァルケイア／無窮の咎',
      gender: '男性',
      age: '17',
      birthday: '2090年10月13日',
      bloodType: 'A',
      height: '192cm',
      weight: '74kg',
      firstPerson: '俺',
      residence: 'オルター・オブ・ザ・ディパーティッド',

      intro: '深いネイビーの髪から覗く赤い瞳、血の気のない死人のような肌をした長身の男。死者の魂を縛って亡霊として使役する異能《Geleit》を操り、彼が支配する「オルター・オブ・ザ・ディパーティッド」は生者が足を踏み入れれば即座に亡霊の仲間入りとなる禁忌の領域。常に困ったように眉を寄せ、言葉は途切れがちで、口調はどこか気弱。だがその全ては、リタのためだけに向けられている。',
    },
    {
      id: 'jelma',
      group: 'valkeia',
      color: '#FFD700',

      name: 'イェルマ',
      realName: 'イェルマ・バルジス',
      alphabet: 'JELMA VALCZYS',
      quote: 'オ？　なんかオモシロそうなことになってんじゃねェか！',

      affiliation: 'ヴァルケイア／無窮の咎',
      gender: '男性',
      age: '？',
      birthday: '2月28日',
      bloodType: 'O',
      height: '196cm',
      weight: '101kg',
      firstPerson: 'オレ/オレサマ',
      residence: 'ディヴェージェンス・ワンダーランド',

      intro: '空間そのものを歪める異能《Distort》の持ち主。重力の向きを分裂させ、触れた物質を再構成し、次元も時間も捻じ曲げる。彼の領域「ディヴェージェンス・ワンダーランド」は、遊びと破壊が一体化した混沌の遊園地であり、何があっても誰も立ち入らない。強い者にしか興味を示さず、「面白い」と思われた者は惨劇の末に死ぬ。純粋な破壊衝動の塊。',
    },

    /* ---------- 皓絳 ---------- */
    {
      id: 'maqi',
      group: 'kokou',
      color: '#FFFFFF',

      name: 'マキ',
      realName: '胡瑪琪',
      alphabet: 'HU MAQI',
      quote: '秩序は、守られている限り誰も傷つけない',

      affiliation: '天衡院 文衡局典司',
      gender: '男性',
      age: '24',
      birthday: '2083年9月21日',
      bloodType: 'O',
      height: '178cm',
      weight: '59kg',
      firstPerson: '私',
      residence: '皓絳',

      intro: '天衡院文衡局の典司。白髪に落ち着いた深紅の瞳を持つ。感情の起伏はほとんどなく、言葉は少なく、口を開けば本質だけを突く。「仕事はするが、無口で無愛想で。近寄りにくい人」と囁かれているが、本人は気付かず過ごしている。',
    },
    {
      id: 'baolong',
      group: 'kokou',
      color: '#9502f7',

      name: 'ポーラン',
      realName: '李保龍',
      alphabet: 'LI BAOLONG',
      quote: '案ずるな。秩序は、我が守る',

      affiliation: '天衡院 武衡局将衡',
      gender: '男性',
      age: '29',
      birthday: '2078年6月6日',
      bloodType: 'O',
      height: '188cm',
      weight: '92kg',
      firstPerson: '我(対武衡局)/私(対文衡局)',
      residence: '皓絳',

      intro: '天衡院の「顔」として民衆の前に立つ、魔法士の頂点。筋肉質な体格にツンツンとした黒髪、鋭い目つきだが、笑うと途端に親しみやすくなる。異能者の襲撃で父と妹を失い、母と姉も奪われかけた過去から、「二度と自分のように家族を失って悲しむ人が生まれない世界」を目指している。マキとリンファを家族同然に思い、二人の前では素のままの気さくな青年に戻る。',
    },
    {
      id: 'linghua',
      group: 'kokou',
      color: '#ff008a',

      name: 'リンファ',
      realName: '沈凌花',
      alphabet: 'SHEN LINGHUA',
      quote: '死なせないわよ。アタシの目の届く範囲では、絶対に',

      affiliation: '天衡院 天医',
      gender: '女性',
      age: '22',
      birthday: '2085年1月29日',
      bloodType: 'B',
      height: '153cm',
      weight: '39kg',
      firstPerson: 'アタシ',
      residence: '皓絳',

      intro: '天衡院唯一の治癒魔法の使い手。幼さの残る顔立ちに、手入れの行き届いたぱっつんボブとマゼンタのインナーカラー。厳格な管理社会にあっても、服装や髪型にわずかな個性を通す。誰に対しても気の強い口調で、文句を言いながら決して手を止めない。治癒魔法によって生じる「歪み」はすべて術者である彼女自身に降りかかるため、内臓はとうにぼろぼろで、血を吐きながら治療にあたることも珍しくない。命綱はシュエンから仕入れる"エナジードリンク"と称した薬。自分の命への執着は驚くほど薄い。',
    },
    {
      id: 'xuean',
      group: 'kokou',
      color: '#ffd772',

      name: 'シュエン',
      realName: '程雪安',
      alphabet: "CHENG XUE'AN",
      quote: 'おや、お探し物で？',

      affiliation: '天衡院 文衡局宰衡',
      gender: '男性',
      age: '34？',
      birthday: '11月5日',
      bloodType: 'A',
      height: '183cm',
      weight: '70kg',
      firstPerson: 'オレ',
      residence: '皓絳',

      intro: '天衡院文衡局宰衡。魔法で生成できない物資の調達と対外交渉を一手に担う、皓絳と外界を繋ぐ唯一のパイプ。服装には常に高級感と機能性を両立させ、龍の刺繍や翡翠のボタンといった意匠をさりげなく取り入れる。とにかく胡散臭く、軽い。交渉、脅し、偽造、情報収集、そして逃亡術――生き延びるための技能には事欠かない。',
    },

    /* ---------- 変異体 ---------- */
    {
      id: 'miko',
      group: 'henitai',
      color: '#fdeff2',

      name: 'ミコ',
      realName: '光泪宮幸仁',
      alphabet: 'YUKIHITO',
      quote: '世界は白光に飲まれ、焼かれ、消えてゆく',

      affiliation: '澄幽・寧静亭',
      gender: '男性',
      age: '16',
      birthday: '2089年4月4日',
      bloodType: 'A',
      height: '162cm',
      weight: '47kg',
      firstPerson: '私',
      residence: '澄幽・寧静亭',

      intro: '澄幽の最奥、寧静亭の禁域で眠り続ける神秘的な変異体。彼の星溶粒子が生む霧と加護が、認識そのものを阻害することで澄幽を外界から隠している。森も渓谷も小川も、澄幽に広がる息を呑むほど美しい風景はすべて彼の力が見せる幻想であり、その力が尽きたとき、この楽園も終わる。未来を視る異能を持ち、目覚めとともに告げられた「白光の滅び」の予言が、物語のすべてを動かし始める。',
    },
    {
      id: 'belyisvet',
      group: 'henitai',
      color: '#a2e8ee',

      name: 'ベリースヴェート',
      realName: 'カテリーナ・スニーシュカ',
      alphabet: 'BELYISVET',
      quote: '',

      affiliation: 'ヴァルケイア中央・エスケープゾーン',
      gender: '女性',
      age: '？',
      birthday: '11月11日',
      bloodType: 'B',
      height: '302cm',
      weight: '計測不能',
      firstPerson: 'わたくし',
      residence: 'ヴァルケイア',

      intro: '東ヨーロッパの廃墟にヴァルケイアを形成した変異体。その力の及ぶ範囲では大気に星溶粒子が満ち、常に青白い光に包まれ、異能者の力が増幅される。信条は「混沌の先にある新たな創造」。中央広場に降り積もる白い雪の結晶ひとつひとつには、彼女が思い描く美しく秩序ある新世界のヴィジョンが宿っている。',
    },
    {
      id: 'obsidia',
      group: 'henitai',
      color: '#000000',

      name: 'オブシディア',
      realName: '趙孤月',
      alphabet: 'OBSIDIA',
      quote: '余が定めた秩序である',

      affiliation: '？',
      gender: '女性',
      age: '？',
      birthday: '12月30日',
      bloodType: 'AB',
      height: '256cm',
      weight: '計測不能',
      firstPerson: '余',
      residence: '？',

      intro: '？？？',
    },

    /* ---------- 澄幽・サブ ---------- */
    {
      id: 'chiyo',
      group: 'choyu',
      color: '#bbbcde',

      name: '藤崎千代',
      realName: '',
      alphabet: 'CHIYO FUZISAKI',
      quote: 'ほら、残さんと食べや。あんた、まだ育つんやから',

      affiliation: '澄幽・翠霞庵 食堂担当',
      gender: '女性',
      age: '32',
      birthday: '2075年10月3日',
      bloodType: 'O',
      height: '171cm',
      weight: '54kg',
      firstPerson: 'うち',
      residence: '澄幽・翠霞庵',

      intro: '澄幽の食を支える、お母さん的存在。柔らかなあずき色の長髪を、左耳の上でざっくりと三つ編みにしているのが特徴。作業時にはたすきをかけて袖をまとめる。京言葉で、穏やかにゆったりと話す。母性に溢れ、誰に対しても分け隔てなく接する面倒見のよさと、場を和ませる明るさを持つ。ただし食べ物を粗末にする者に対しては豹変し、凄まじい圧力で詰め寄る。自分には戦う力がない。だからせめて、外へ出ていく者たちの腹を満たし、帰ってくる場所を温かく保っておきたい。',
    },
    {
      id: 'soshin',
      group: 'choyu',
      color: '#4a5a4e',

      name: '葛城宗真',
      realName: '',
      alphabet: 'SOSHIN KATSURAGI',
      quote: 'テメエ、また無茶しやがったな。そこに座れェ！！',

      affiliation: '澄幽・翠霞庵 療養所担当',
      gender: '男性',
      age: '78',
      birthday: '2029年1月10日',
      bloodType: 'A',
      height: '172cm',
      weight: '62kg',
      firstPerson: '儂',
      residence: '澄幽・翠霞庵',

      intro: '澄幽唯一の医師。療養所を預かり、住民の傷も病も一手に引き受ける。口が悪く、患者を「テメエ」と呼び、無茶をして帰ってきた者には容赦のない怒声が飛ぶ。珠桜が「呼んでこようか？」と口にしただけでルイが焦るほど、澄幽で最も恐れられている人物。それでも彼の手が誰かを見放したことは一度もない。',
    },
    {
      id: 'xin',
      group: 'choyu',
      color: '#B03A28',

      name: 'シン',
      realName: '辛燚',
      alphabet: 'XIN YI',
      quote: 'はあ？　壊した……？　おいテメエ表出ろ夜霧、この世に二つとない芸術だったのにいいいい！！！',

      affiliation: '澄幽・試刃院 武器担当',
      gender: '男性',
      age: '134',
      birthday: '1973年8月19日',
      bloodType: 'B',
      height: '180cm',
      weight: '66kg',
      firstPerson: 'オレ',
      residence: '澄幽・試刃院',

      intro: '澄幽随一の異能技術者。材料と星骸の核から異能武器を制作し、既存兵器の調整も手掛ける。珠桜に次ぐと言われる美貌の持ち主だが、作業着の袖も裾も大抵焦げている。基本は無口。ただし武器の話になると勢いのままに喋り倒し、周囲を置き去りにするので変態扱いされている。',
    },

    /* ---------- サンクチュアリ・オブ・リバース ---------- */
    {
      id: 'kirke',
      group: 'valkeia',
      color: '#a04a3c',

      name: 'キュルケー',
      realName: 'キュルケー・ランヴェルトス',
      alphabet: 'KIRKĒ LAMBERTOS',
      quote: '……戯言は慎みなさい',

      affiliation: 'サンクチュアリ・オブ・リバース メイド長',
      gender: '女性',
      age: '非公開',
      birthday: '2058年10月19日',
      bloodType: 'A',
      height: '172cm',
      weight: '56kg',
      firstPerson: '私',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: 'ティフォン家に古くから仕えるメイド長。几帳面で厳格、そして驚異的な忍耐力の持ち主。屋敷とミレディーナの願いを何よりも重んじる。異能《Palimpsest》は永劫回帰――自身の「状態」を任意の時点で保存し、後からその時点へ時間軸ごと巻き戻す。完璧な立ち居振る舞いを崩さないが、実はヘビースモーカーで、巻き戻しを使ってまで煙草の匂いを必死に消している。',
    },
    {
      id: 'philia',
      group: 'valkeia',
      color: '#f28cb1',

      name: 'フィリア',
      realName: 'フィリア・ドゥ・コルド',
      alphabet: 'PHILIA DU CORDE',
      quote: '当主様にぴったりなお洋服をご用意いたします',

      affiliation: 'サンクチュアリ・オブ・リバース 衣装係',
      gender: '女性',
      age: '18',
      birthday: '2089年6月22日',
      bloodType: 'B',
      height: '158cm',
      weight: '46kg',
      firstPerson: 'わたくし',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: '異能《Fil》によって、細くしかし切断不可能なほど強靭な糸を操る。銀の縫い針を媒体とし、数メートル離れた場所からドレスの着脱やホック止めを一瞬で完了させる。カーテンやタペストリーを操って室内の印象を一変させ、ミレディーナを楽しませる。だが糸を通した相手は、関節の可動すら無視して、真にマリオネットのごとく意のままに動かされる。ミレディーナの「気に入らない人形」を掃除するのも、彼女の役目。魔法少女とロリータファッションが好き。',
    },
    {
      id: 'eucleia',
      group: 'valkeia',
      color: '#8fb8c8',

      name: 'エウクレイア',
      realName: 'エウクレイア・ストレフサ',
      alphabet: 'EUCLEIA STREPSA',
      quote: '掃除は、まだ終わっておりません',

      affiliation: 'サンクチュアリ・オブ・リバース 清掃担当',
      gender: '女性',
      age: '秘密',
      birthday: '2060年7月10日',
      bloodType: 'O',
      height: '165cm',
      weight: '51kg',
      firstPerson: 'わたくし',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: 'アイスブルーの髪を持つ、ティフォン家古参のメイド。几帳面で、清潔への執着は異常の域にある。汚れを、絶対に許さない。異能《Strephō》は特定の空間の重力方向と認識そのものを捻じ曲げ、侵入者を強烈な空間酔いと方向感覚の喪失に陥らせる。掃除にもこれを持ち出すため、固定されていない家具を壊してはキュルケーに何百回と叱られている。匂いフェチで、屋敷のアロマも担当。',
    },
    {
      id: 'natasha',
      group: 'valkeia',
      color: '#5a7a5e',

      name: 'ナターシャ',
      realName: 'ナターシャ・ヴォズヴラシェーニエ',
      alphabet: 'NATASHA VOZVRASHCHENIYE',
      quote: 'なぜ、あなた方はいつも、私たちから大切なものを奪うのですか？',

      affiliation: 'サンクチュアリ・オブ・リバース 治療担当',
      gender: '？',
      age: '21',
      birthday: '2086年9月2日',
      bloodType: 'O',
      height: '162cm',
      weight: '50kg',
      firstPerson: '私',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: '常ににこにこと満面の笑みを浮かべている治療係。異能《Vozvrashcheniye》は、触れた物体や生物の汚れ、破損、老化、怪我を「無かったこと」にし、任意の過去の状態へ戻す力。壁の傷を修復し、古びた家具を新品同然にし、負傷した使用人の傷を瞬時に癒す。ただし完全に消滅したものは復元できない。──なお、この力は人を幼児まで巻き戻せる。彼女は狂気のショタコンである。',
    },
    {
      id: 'medea',
      group: 'valkeia',
      color: '#e0dbe4',

      name: 'メディア',
      realName: 'メディア・アナグノシス',
      alphabet: 'MEDEA ANAGNOSIS',
      quote: '従順なる狗は、主人の前で大人しくしているものです……ね？',

      affiliation: 'サンクチュアリ・オブ・リバース メンタルケア担当',
      gender: '女性',
      age: 'ひみつ♡',
      birthday: '5月15日',
      bloodType: 'AB',
      height: '168cm',
      weight: '52kg',
      firstPerson: 'わたくし',
      residence: 'サンクチュアリ・オブ・リバース',

      intro: '慈愛に満ちた微笑みを常に絶やさない。だがその本性は、他人の心の最も深く脆い部分に触れ、意のままに操ることに至上の喜びを見出すもの。異能《Psyche》は言葉や視線を媒介に、相手の思考、感情、記憶を読み取り、書き換え、強烈な暗示を植え付ける。ミレディーナに敵意を持つ者から情報を搾取する尋問官として、あるいはわずかな不安と猜疑心を増幅させて組織を内部から瓦解させる工作員として、その力を振るう。',
    },
  ],
};
